...
def retorna_documentos_processo_completo(num_processo, cpf=None, senha=None):
    """
    Retorna todos os documentos de um processo, incluindo conteúdo.
    ...
    Returns:
        dict: Documentos do processo com conteúdo ou mensagem de erro
    """
    try:
        logger.debug(f"\n{'=' * 80}")
        logger.debug(f"Buscando todos os documentos do processo {num_processo}")
        logger.debug(f"{'=' * 80}\n")
        
        # 1. Consultar o processo para obter todos os documentos (metadados, sem conteúdo)
        ...
        # 2. Extrair todos os documentos e seus metadados
        docs = resposta_processo.processo.documento
        if not isinstance(docs, list):
            docs = [docs]
            
        # Função para extrair recursivamente todos os documentos
        def extract_all_documents(doc_list):
            all_docs = []
            
            for doc in doc_list:
                # Adiciona o documento atual
                doc_info = {
                    'id_documento': getattr(doc, 'idDocumento', ''),
                    'tipo_documento': getattr(doc, 'tipoDocumento', ''),
                    'descricao': getattr(doc, 'descricao', ''),
                    'data_hora': getattr(doc, 'dataHora', ''),
                ...
                if hasattr(doc, 'documentoVinculado'):
                    ...
                    if not isinstance(vinc_docs, list):
                        vinc_docs = [vinc_docs]
                    all_docs.extend(extract_all_documents(vinc_docs))
                
                # Processa outros tipos de documentos associados
                for attr in ['documento', 'documentos']:
                    if hasattr(doc, attr):
                        outros = getattr(doc, attr)
                        if outros:
                            outros_list = outros if isinstance(outros, list) else [outros]
                            all_docs.extend(extract_all_documents(outros_list))
            
            return all_docs
        
        # Extrai todos os documentos
        documentos = extract_all_documents(docs)
